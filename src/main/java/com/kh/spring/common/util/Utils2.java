...
public class Utils2 {
	...
	public static String getPageBar(int totalContent , int cPage , int numPerPage ,String type,String search ,String pageName) {
...
//		while(!(pageNo > pageEnd || pageNo > totalPage)) 아래와 같은 조건
		while(pageNo<=endPage && pageNo<=totalPage) {
			if(cPage==pageNo) {
				...
				pageBar += "<li class='page-item'><a class = 'page-link' href='"+pageName+"?cPage="+pageNo+"&numPerPage="+numPerPage+"&type="+type+"&search="+search+"'>"+pageNo+"</a></li>";
			}
			pageNo++;
		}
